...
class cal extends Component {
  ...
  componentDidMount() {
    ...
    axios.post('https://pocketplanner-api.railway.internal/api/calendar', { token: token })
        .then(response => {
  ...
  handleSelect = async ({ start, end }) => {
    ...
    if (title) {
      ...
      const response = await fetch('https://pocketplanner-api.railway.internal/api/calendar/createEvent', {
        method: 'POST',
  ...
  handleSelectEvent = async (eventInfo) => {
    if(window.confirm(`Are you sure you want to delete the event '${eventInfo.title}'`)) {
      ...
      const response = await fetch('https://pocketplanner-api.railway.internal/api/calendar/' + id, {
        method: 'DELETE'
      })
      const json = await response.json();
    }
